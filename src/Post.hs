...
import Util ((.*), getTitleText, readDoc, readMetadata, uniqDesc)
...
postDataToMetaValue (PostData (Title inlines) slug) =
...
buildSlugLookup getMetadata postPaths = do
  ...
  where
    ...
    toTitleSlugPair postData = (title postData, getSlug postData)
...
extractLinks slugLookup readerOptions srcPath =
...
buildLinkGraph getMetadata slugLookup readerOptions postPaths = do
  postDatas <- traverse (getPostData getMetadata) postPaths
  links <- traverse (extractLinks slugLookup readerOptions) postPaths
